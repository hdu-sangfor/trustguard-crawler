...
- DROP DATABASE因外键约束静默失败 → 下一个Payload在DROP前加SET GLOBAL FOREIGN_KEY_CHECKS=0，执行后重置
这不是重试，这是理解失败模式后的针对性修正。
04 勒索执行：1342条配置的加密与毁灭
控制Nacos后，JADEPUFFER执行了勒索核心动作：
使用MySQL的AES_ENCRYPT()函数，加密了全部1342条Nacos服务配置项，随后删除原始的config_info和历史表，创建了一张名为README_RANSOM的勒索表，内含勒索声明、比特币支付地址和Proton Mail联系方式。
